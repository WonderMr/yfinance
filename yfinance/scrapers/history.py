...
class PriceHistory:
    ...
    def _prepare_request_params(self, period, interval, start, end, repair):
        ...
        if repair and interval in ["5d", "1wk", "1mo", "3mo"]:
            ...
            if start is None and end is None and period is not None:
                if period == "ytd":
                    ...
                    start = (
                        now.date()
                        - utils._interval_to_timedelta(period)
                        - _datetime.timedelta(days=4)
                    )
                period = None
            interval = "1d"
        ...
        if period is None:
            ...
            if period.lower() == "max":
                ...
                if start is None:
                    if interval == "1m":
                        start = end - _MAX_PERIOD_1M_SECONDS
                    elif interval in ("2m", "5m", "15m", "30m", "90m"):
                        start = end - _MAX_PERIOD_INTRADAY_SECONDS
                    elif interval in ("1h", "60m"):
                        start = end - _MAX_PERIOD_1H_SECONDS
                    else:
                        start = end - _MAX_PERIOD_DAILY_SECONDS
            elif start and end:
                raise ValueError("Don't set period, start and end.")
            elif start or end:
    ...
    def _parse_and_clean_base_data(self, data, interval, end_dt, prepost):
        ...
        if (
            end_dt
            and not quotes.empty
            and quotes.index[-1] >= end_dt.tz_convert("UTC").tz_localize(None)
        ):
        ...
        if (
            not prepost
            and interval[-1] in ("m", "h")
            and "tradingPeriods" in self._history_metadata
        ):
    ...
    def _combine_dataframes(
        ...
        if dividends is not None and dividends.shape[0] > 0:
            df = utils.safe_merge_dfs(df, dividends, interval)
        if splits is not None and splits.shape[0] > 0:
            df = utils.safe_merge_dfs(df, splits, interval)
        expect_capital_gains = self._history_metadata.get("instrumentType") in (
            "MUTUALFUND",
            "ETF",
        )
        if (
            expect_capital_gains
            and capital_gains is not None
            and capital_gains.shape[0] > 0
        ):
    ...
    def history(
        ...
        logger = utils.get_yf_logger()
        params, interval_user, period_user, start_dt, end_dt = (
            self._prepare_request_params(period, interval, start, end, repair)
        )
        ...
        if not (
            ...
            and data["chart"]["result"][0]
        ):
            if raise_errors:
                raise YFPricesMissingError(
                    self.ticker, f"No data found, symbol may be delisted"
                )
            logger.error(f"{self.ticker}: No data found, symbol may be delisted")
            return utils.empty_df()

        quotes, dividends, splits, capital_gains = self._parse_and_clean_base_data(
            data, current_interval, end_dt, prepost
        )
        df = self._combine_dataframes(
            quotes,
            dividends,
            splits,
            capital_gains,
        ...
        if df.empty:
            logger.debug(
                f"{self.ticker}: No data after processing, returning empty DataFrame"
            )
            return df
        ...
        try:
            ...
            method = "auto_adjust" if auto_adjust else "back_adjust"
            err_msg = f"{method} failed: {e}"
            if raise_errors:
                raise Exception(f"{self.ticker}: {err_msg}") from e
            logger.error(f"{self.ticker}: {err_msg}")
            return utils.empty_df()

        if rounding:
            df = np.round(df, self._history_metadata.get("priceHint", 2))
        ...
        if _retry and mask_nan.any():
            ...
            pass
        if not keepna:
            df = df.drop(mask_nan.index[mask_nan])
        ...
        logger.debug(
            ...
            if not df.empty
            else f"{self.ticker}: yfinance returning OHLC: EMPTY"
        )
    ...
    def _fix_bad_stock_splits(self, df, interval, tz_exchange):
        ...
        splits_info = str(df["Stock Splits"][split_f].to_dict())
        logger.debug(f"Splits: {splits_info}", extra=log_extras)
